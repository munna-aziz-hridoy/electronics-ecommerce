...
const OrderTR = ({ order, refetch }) => {
  ...
  return (
    <tr className="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
      ...
      <td className="px-6 py-2">${total_price}</td>
      <td className="px-6 py-2">{total_products}</td>
      <td className="px-6 py-2">{delivery ? "Delivered" : "In progress"}</td>
      <td className="px-6 py-2">{paid ? "Paid" : "Unpaid"}</td>
